...
lumera run scripts/seed-demo.py        # Seed customers, invoices, activities, and payments
```
...
lumera apply automations               # Deploy only automations (e.g. after editing ca_assess_risk)
lumera apply hooks                     # Deploy only hooks (e.g. after editing audit hooks)
...
```bash
lumera run agents/collections_assistant "Which customers have overdue invoices?"   # Invoke an agent
lumera run agents/collections_assistant "Draft an email" --session my-session      # Continue conversation
```
...
run = automations.run_by_external_id("collections-agent:ca_assess_risk", inputs={"customer_id": "abc123"})
